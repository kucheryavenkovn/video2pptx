# FILE: src/video_slide_md/notes_pipeline.py
# VERSION: 0.1.0
...
from video_slide_md.subtitles import parse_subtitles, align_cues_to_segments
...
def run_notes(
    ...
    if notes_mode == "llm":
        logger.info("[Notes][run_notes] LLM mode requested — need llm_client wired from CLI")

    for seg in doc.slides:
        cleaned = process_notes(seg, mode=notes_mode)
