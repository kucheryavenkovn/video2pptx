...
- Date: 2026-07-14
- Area: detection, tooling
- Finding: Corrected Step 18.4C-r2 established REFERENCE_EXACTLY_REPEATABLE across three fresh complete 1,201-frame production-equivalent opens, so exact cross-open byte discrimination is valid. C1 is NOT_VIABLE_EXACT_PARITY_FAIL for the exact historical seek prototype, with ROOT_CAUSE_UNKNOWN_NOT_ISOLATED. C2 proves an exact rolling retention model and exact same-run/downstream semantics with a measured peak of 15 frames / 93,312,000 bytes, but fails performance (reference median 290.526s, candidate median 330.722s, median paired reduction -16.115%, not directionally faster). C3 deterministically selected thread_count_1, thread_count_4, and thread_count_8; all pass exact sequence parity but fail performance (median paired reductions -2.029%, -4.571%, +0.944%; none directionally faster or at least 15%).
...
- Impact: All corrected candidate discriminators are valid, but no targeted optimization satisfies exact semantics plus the stable >=15% performance gate. Outcome T3 is BLOCKED_NO_EVIDENCE_SUPPORTED_TARGET_OPTIMIZATION. selected_optimization=NONE; Step 18.4 remains in_progress; Step 18.5 remains planned/blocked and not started.
- Resolution/Status: OPEN. A new evidence-supported candidate class or materially changed evidence is required before Step 18.5 can begin. The C2 retain-all value is an upper bound, C1 causality is not isolated, and C3 thread settings are measured rather than presumed ignored.
- LINKS: M-DETECT-PERF-DECISION, M-DETECT-TARGET-DISCRIMINATION-R2, V-PERF-DETECT-TARGET-OPTIMIZATION-DISCRIMINATION, benchmarks/detect/evidence/target-optimization-discrimination-r2-20260714-0bcfe54/
