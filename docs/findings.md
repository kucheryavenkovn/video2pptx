...
### F-0096 — Step 18.4 draft at 27a28bf claimed FEATURE_EXTRACTION_CPU with incorrect evidence interpretation (REJECTED)
- Date: 2026-07-13
- Area: detection
- Finding: Commit 27a28bf was a Step 18.4 bottleneck decision draft that claimed FEATURE_EXTRACTION_CPU as the primary bottleneck and opencv-accelerated-histogram-gray as the selected optimization. This classification was rejected during acceptance review because it used incorrect wall-clock values (pass2_collect=10.9s instead of 65.2s, visual_distance=12.4s instead of 0.28s), fabricated an asynchronous producer-consumer pipeline that does not exist in the current synchronous generator-based detection loop, used wrong cProfile values (extract_features=107.3s instead of 92.7s, phash=9.9s instead of 3.9s, dhash=8.5s instead of 3.1s), and misstated F-0088 as run variance (F-0088 is 28 slides / 6 PNGs).
- Symptom/Reproduction: 27a28bf contains bottleneck_decision.json and bottleneck_decision.md with the rejected classification.
- Impact: FEATURE_EXTRACTION_CPU is not accepted by F-0096. opencv-accelerated-histogram-gray is not accepted by F-0096. The draft is preserved in repository history as REJECTED_STEP_18_4_DRAFT.
- Resolution/Status: Rejected during acceptance audit on 2026-07-13. Step 18.4 remains in_progress. Corrected decision in subsequent commit.
- LINKS: M-DETECT-PERF-DECISION, V-PERF-DETECT-BOTTLENECK, 27a28bf

### F-0097 — Step 18.4 lacks wall-clock instrumentation for first-pass decode; bottleneck cannot be safely discriminated
- Date: 2026-07-13
- Area: detection
...
- Date: 2026-07-13
- Area: detection
- Finding: Step 18.4 has selected DECODE_FRAME_PIPELINE with HIGH confidence, but the proposed PYAV_HWACCEL_CUDA optimization is invalid because current source already requests HWAccel via _pick_hw_device() → _create_hwaccel_with_evidence() → av.open(hwaccel=...). Accepted benchmark evidence (hermes-600s-f0097-instrumented-20260713-acb424f) does not record actual HWAccel activation state (requested_hw_device, hwaccel_object_created, container_opened_with_hwaccel, codec_context_is_hwaccel, actual_hardware_decode_active, software_fallback_detected, software_fallback_reason, codec_name, codec_long_name, hardware decoder/device identity where PyAV exposes it). Without this evidence, a targeted decoder optimization cannot yet be selected safely. Multiple candidates exist (observability, second-pass elimination, decoder tuning) but none is sufficiently supported.
- Symptom/Reproduction: Source audit at candidate HEAD proves existing source already: (1) _HW_PREFERRED begins with "cuda", (2) pyav_iter_frames calls _pick_hw_device() → _create_hwaccel_with_evidence() → av.open(hwaccel=hwaccel). No committed benchmark artifact records whether hardware decode was actually active at runtime. Previous decision (PYAV_HWACCEL_CUDA) was based on a false model of existing source state.
...
- Resolution/Status: RESOLVED (activation-defect discrimination) — current state; resolved by Step 18.4B (see the "Step 18.4B resolution" bullet below).
- Historical Step 18.4A intermediate state (before Step 18.4B): PARTIALLY_RESOLVED (2026-07-14 canonical Hermes H.264 evidence run, outcome OUTCOME_D). Evidence mechanism implemented and corrected (ST-18.4A, 2026-07-13): (1) private evidence observer separating raw codec_context_is_hwaccel from actual_hardware_decode_active, (2) _create_hwaccel_with_evidence capturing creation errors, (3) deterministic generator cleanup, (4) provenance-validated probe (--accepted-base, --canonical-mode). Tests: 17/17 pass. Canonical Hermes H.264 clip (SHA256 dd9da344...) was located and the probe executed (see F-0100 RESOLVED). Canonical observations: requested_hw_device=cuda, hwaccel_object_created=true (no creation error), container_opened_with_hwaccel=true, raw codec_context_is_hwaccel=true, allow_software_fallback=true, codec_name=h264 (1920x1080, 60fps), first_frame_shape=[1080,1920,3], observation_consistency=true, status OBSERVATION_COMPLETE. However actual_hardware_decode_active=UNKNOWN_NOT_PROVEN by observer design (installed API does not expose actual HW decode vs configured HWAccel) — actual-HW-decode conclusion confidence = NOT_ESTABLISHED while canonical observation completeness confidence = HIGH. The strict fallback control (allow_software_fallback=false) is INVALID (INVALID_SUPPORTING_CONTROL_IMPLEMENTATION_DEFECT: premature outer break after the first demuxed packet, so for this H.264 stream the first packet yields no frame and it reports STRICT_PROBE_NO_FRAME); it provides no runtime signal, is NOT filed as a separate finding, and must be corrected (demux until first decoded frame / EOF / exception) before the next HWAccel evidence decision. Canonical evidence commit: 9b4cc54949537b2a36e70a0f98f1cb732d6688b2; evidence dir benchmarks/detect/evidence/hwaccel-runtime-hermes-h264-20260714. Gap narrowed (runtime evidence object now exists and is complete) but the activation-determination question remains open. Raw artifact known-limitation omission: 9b4cc54 reports evidence_confidence=HIGH / limitations=[] which the decision layer qualifies (HIGH = observation completeness only; NOT_ESTABLISHED for actual HW decode). selected_optimization=NONE; Step 18.4A=done; Step 18.4=in_progress; Step 18.5=planned/blocked.
- Step 18.4B resolution (2026-07-14): RESOLVED for the activation-defect discrimination. The corrected strict no-software-fallback control (HWAccel('cuda',0), allow_software_fallback=false) on the exact canonical clip produced FIRST_FRAME_DECODED (packets_examined=3, packets_with_decoded_frames=1, frames_decoded=1, frames_converted=1, first_frame_timestamp=0.033, first_frame_shape=[1080,1920,3], container_opened=true, container_closed=true; strict_control_evidence_commit ef589a12187c299f4bd086dc690a38b3f7095982; strict-control code HEAD 04c0dc1f21cd8f37a9c82925e9c1470a40759553 tree b93329dde6740d2f7f0283e6c7fb2235f0245bb3; accepted master base 547dc06ef6bfe331daf60018ac45bd342f0d3dfd). The corrected control demuxes until the first decoded frame / EOF / exception (does NOT stop after an empty packet) and explicitly closes the container in a finally block. Bounded fact: the current environment supports decoding at least one canonical H.264 frame through a CUDA HWAccel configuration with software fallback explicitly disabled. No CUDA activation/fallback defect is proven; PYAV_HARDWARE_DECODE_ACTIVATION_OR_FALLBACK_FIX is contradicted as a targeted optimization. NOT proven: that all production benchmark frames used hardware decode, that the production fallback-enabled path never used software fallback, or any throughput improvement. Production-path actual_hardware_decode_active remains UNKNOWN_NOT_PROVEN by observer design (not needed to discriminate the optimization). selected_optimization=NONE; Step 18.4B=done; Step 18.4=in_progress; Step 18.5=planned/blocked. No new finding (the corrected control succeeded; no new gap).
- LINKS: M-DETECT-PERF-DECISION, V-PERF-DETECT-BOTTLENECK, F-0100, pyav_backend.py, tools/probe_pyav_hwaccel.py

### F-0100 — Canonical Hermes H.264 clip not available on local workstation
- Date: 2026-07-13
- Area: detection, tooling
- Finding: The canonical benchmark evidence object (examples/hermes-0000-1000.mp4, SHA256 dd9da3442e91ab7f17f0405198aa8e39d1538d74518b6b9a3b1e61ac2fc0f5a4, H.264, 1920x1080, ~600s) was not present on the current workstation at the paths originally searched (D:\git\video2pptx\examples\, D:\git\worktrees\*\examples\, common download/video directories). The file is intentionally gitignored (*.mp4) and must be obtained separately.
- Symptom/Reproduction: Step 18.4A probe could not be executed with canonical evidence because the clip file did not exist at the previously searched paths.
- Impact: (Historical) F-0098 could not be closed and Step 18.4A could not advance past in_progress / blocked while the clip was missing.
- Resolution/Status: RESOLVED (2026-07-14). The exact canonical clip (SHA256 dd9da3442e91ab7f17f0405198aa8e39d1538d74518b6b9a3b1e61ac2fc0f5a4) was found in three byte-identical candidate copies in other worktrees: D:\git\worktrees\video2pptx-phase18-bottleneck-instrumentation\examples\hermes-0000-1000.mp4, D:\git\worktrees\video2pptx-phase18-short-benchmark\examples\hermes-0000-1000.mp4, and D:\git\worktrees\video2pptx-phase18-short-benchmark\hermes-0000-1000.mp4 (all SHA256 = dd9da344..., unique hash count=1). The clip was copied (Copy-Item, source preserved, not Move-Item) local-only into the canonical-evidence worktree examples\hermes-0000-1000.mp4. It is gitignored by *.mp4 (.gitignore:83) and was NOT committed. Media identity verified: h264 / H.264 / AVC / MPEG-4 AVC / MPEG-4 part 10, 1920x1080, 60/1 fps, 600.016667 s, 36001 frames. Canonical probe then executed successfully (OBSERVATION_COMPLETE).
- LINKS: M-DETECT-PERF-DECISION, V-PERF-DETECT-BOTTLENECK, F-0098, tools/probe_pyav_hwaccel.py

### F-0102 — Rejected Step 18.4C draft finding
- Date: 2026-07-14
- Area: detection, tooling
- Finding: REJECTED_STEP_18_4C_DRAFT_FINDING. This entry records the rejected decision context from 4b6eba59467110642e0959407d7bec9ff59ac7d8 and is not a current project finding. Its terminal conclusion depended on a C3 code-to-artifact provenance contradiction, unsupported C1 causal attribution, and a C2 retain-all upper bound mislabeled as a required minimum.
- Symptom/Reproduction: The committed C3 inspection marks thread_count and thread_type writable and POTENTIALLY_PARITY_PRESERVING, and the committed code constructs variants, but the committed artifact records approved_variants=[] and NO_EVIDENCE_SUPPORTED_CONFIGURATION_VARIANT. The C1 experiment varied seek, container lifetime, decoder history, and codec-context access together. The C2 arithmetic measured only 1201 full RGB frames retained simultaneously.
- Impact: Outcome T3 from 4b6eba5 is not accepted. Step 18.4C is in_progress, selected_optimization=NONE, and Step 18.5 remains planned/blocked and not started. Historical C1 0/84 exact matches and the C2 7,471,180,800-byte retain-all upper bound remain preserved without causal promotion.
- Resolution/Status: REJECTED_STEP_18_4C_DRAFT_FINDING / NOT CURRENT. Corrected evidence uses a new r2 directory and, if a new current finding is required, F-0103.
- LINKS: M-DETECT-PERF-DECISION, V-PERF-DETECT-BOTTLENECK, F-0097, F-0098, F-0100, tools/probe_pass2_retrieval.py, tools/probe_pyav_decode_config.py, tools/discrimination_helpers.py

### F-0103 — Corrected discrimination found no viable DECODE_FRAME_PIPELINE target
- Date: 2026-07-14
- Area: detection, tooling
- Finding: Corrected Step 18.4C-r2 established REFERENCE_EXACTLY_REPEATABLE across three fresh complete 1,201-frame production-equivalent opens, so exact cross-open byte discrimination is valid. C1 is NOT_VIABLE_EXACT_PARITY_FAIL — causal_classification=ROOT_CAUSE_UNKNOWN_NOT_ISOLATED (codec_context_causal=false, seek_causal=false); the causal matrix was NOT run, so no isolated root cause is claimed. The historical benchmark path (HEAD acb424f) did NOT access stream.codec_context in pyav_iter_frames while the current production-equivalent path DOES — a proven code-path difference and the LEADING HYPOTHESIS (codec_context_access=LEADING_HYPOTHESIS_NOT_PROVEN) for the canonical-signature change. Causality is NOT isolated because no controlled A/B varied ONLY stream.codec_context access and other HWAccel/evidence infrastructure changed together with it. C2 is NOT_VIABLE_PERFORMANCE_FAIL (corrected from NOT_VIABLE_RESOURCE_MODEL) — rolling-window exact model IS proven (peak 15 frames / 93,312,000 bytes), but candidate is slower (reference median 290.5263571000032s, candidate median 330.72188989999995s; median of paired differences -46.81963299999916s / -16.115451096192%; difference of medians -40.19553279999673s / -13.835416931263438%; not directionally faster). C3 is NOT_VIABLE_PERFORMANCE_FAIL — thread_count_1/4/8 selected and pass exact sequence parity but fail performance (median paired reductions -2.029%, -4.571%, +0.944%; none directionally faster or >= 15%). Canonical signature provenance: fresh runs produce 5a7c4538... while accepted immutable is 8cc06c6a...; match=false; provenance classification ROOT_CAUSE_UNKNOWN_NOT_ISOLATED (the stream.codec_context code-path difference is a leading hypothesis (codec_context_access=LEADING_HYPOTHESIS_NOT_PROVEN); causality is NOT isolated; environment/library/runtime contribution is not excluded; a controlled single-variable A/B varying ONLY stream.codec_context access is still required).
- Symptom/Reproduction: Run the committed r2 probes from clean code HEAD 0bcfe540c62b5d1130ba2dbb3d067c46234062bb (tree a55759461374ac747b1e87e1611c544ce59ca969) on canonical Hermes SHA256 dd9da3442e91ab7f17f0405198aa8e39d1538d74518b6b9a3b1e61ac2fc0f5a4. Raw evidence commit b4ed0e40c2f9d86c0db41c2dd53106f945f2502c.
- Impact: All corrected candidate discriminators have valid terminal measurements. C2 taxonomy corrected from resource model failure to performance failure. No targeted optimization satisfies exact semantics plus the stable >=15% performance gate. Management decision (Step 18.4 closeout): the negative terminal outcome T3_NO_EVIDENCE_SUPPORTED_TARGET_OPTIMIZATION is accepted — no tested C1/C2/C3 candidate is viable, so selected_optimization=NONE. The canonical signature provenance discrepancy (8cc06c6a... vs 5a7c4538...) remains OPEN under this finding as NON-BLOCKING technical debt and does not reopen the completed candidate-discrimination result. Step 18.4C=done; Step 18.4=done; Step 18.5=planned/blocked and not started (implementation_started=false).
- Resolution/Status: OPEN / NON_BLOCKING. The candidate-discrimination step is complete (accepted terminal outcome T3_NO_EVIDENCE_SUPPORTED_TARGET_OPTIMIZATION); this finding no longer blocks Step 18.4 closeout. It remains open as technical debt. Two paths to fully resolve the provenance gap: (1) run a controlled causal A/B probe that varies ONLY stream.codec_context access in pyav_iter_frames (removing it is NOT sufficient by itself, because its causality is not yet proven), or (2) make an architectural decision on the production decode path and then explicitly, consciously re-baseline the immutable signature under the current code. Environment/library/runtime contribution is not excluded. The evidence_observer added to slide_detector.py receives mutable cropped ndarray and must be documented as a trusted intrusive diagnostic hook.
- LINKS: M-DETECT-PERF-DECISION, M-DETECT-TARGET-DISCRIMINATION-R2, V-PERF-DETECT-TARGET-OPTIMIZATION-DISCRIMINATION, benchmarks/detect/evidence/target-optimization-discrimination-r2-20260714-0bcfe54/, pyav_backend.py:230

### F-0104 — Analysis resolution not discriminated in Phase 18
- Date: 2026-07-18
- Area: detection, tooling, performance
- Finding: Phase 18 bottleneck instrumentation shows extract_features ≈34% and decode/frame pipeline ≈63% of wall-clock on Hermes 600s, but feature extraction already collapses frames to 48×48 gray_thumb (80% of visual_distance weight) plus small hashes/hist. Full-resolution RGB is still fed into extract_features after ROI, while export screenshots legitimately need full-res in Pass 2. Phase 18 candidates C1 (seek), C2 (rolling retention), C3 (thread_count) did not include analysis_max_side / analysis downscale discrimination. Therefore a separate optimization class remains open without reopening Phase 18 T3 / selected_optimization=NONE.
- Symptom/Reproduction: Read frame_features.py (48×48 thumb, DISTANCE_WEIGHTS pixel_mse=0.80); read bottleneck_decision.md stage table; note absence of analysis_max_side in config.py VideoConfig and in Step 18.4C candidate list.
- Impact: Untested lever for cutting feature CPU and RGB transfer without reducing PNG quality if dual-res path is used. Risk of overstating decode-only optimization space.
- Resolution/Status: RESOLVED for discrimination — Phase 19 Hermes sweep (2026-07-18, HEAD 5111833) proves analysis_max_side helps: native wall 243.85s → 480: 113.25s (−53.6%, ~2.15×); extract_features 85.95→5.67s (−93.4%); full-res PNG OK; quality gates PASS at 480 vs native. 960/640/320 fail false_split (6 vs 4 slides). Default remains null; recommended 480 documented in config.example.yaml. Caveat: quality is native-relative (native only 4 long segments under min_slide_duration=10), not absolute GT. p1_decode also dropped (103→52s) partly order/cache — do not claim pure decode optimization. Evidence: phase19-hermes-600s-20260718/.
- LINKS: M-ANALYSIS-SCALE, M-GOLDEN-MEAN-SWEEP, M-GOLDEN-MEAN-DECISION, M-PHASE19-PLAN, M-FRAME-FEATURES, M-DETECT-SLIDES, V-M-PHASE19-PLAN, V-M-ANALYSIS-SCALE, V-M-DETECT-ANALYSIS-SCALE, V-M-GOLDEN-MEAN-SWEEP, V-M-GOLDEN-MEAN-DECISION, F-0103, benchmarks/detect/evidence/phase19-hermes-600s-20260718/golden_mean_decision.md

### F-0105 — analysis_max_side reduces extract_features and wall-clock on Hermes 1080p
- Date: 2026-07-18
