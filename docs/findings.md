...
### F-0102 — No evidence-supported target optimization for DECODE_FRAME_PIPELINE
- Date: 2026-07-14
- Area: detection, tooling
- Finding: Step 18.4C target optimization discrimination evaluated three remaining candidates against the accepted DECODE_FRAME_PIPELINE bottleneck (172.46s, 63.27%, HIGH confidence). None satisfied all viability requirements (exact parity, bounded intervention, resource safety, measured benefit). C1 (PASS2_TARGETED_REPRESENTATIVE_FRAME_RETRIEVAL): exact frame parity FAIL (0/84 targets) — the production decode path (pyav_iter_frames with stream.codec_context access) produces different pixel values than any alternative decode path (direct av.open + seek + decode) for the same frame; this is a HW decode environment-specific behavior (PyAV 18.0.0 / NVDEC / CUDA). C2 (PASS1_SAMPLE_FRAME_RETENTION): no bounded O(number_of_segments) retention model exists; full retention requires ~7.47 GB with no project memory budget. C3 (PYAV_DECODE_CONFIGURATION_TUNING): only thread_count/thread_type are writable and potentially parity-preserving, but both are ignored by NVDEC hardware decode — no evidence-supported variant exists.
- Symptom/Reproduction: Diagnostic probes (tools/probe_pass2_retrieval.py, tools/probe_pyav_decode_config.py) executed on the exact canonical Hermes H.264 clip (SHA dd9da344...). Evidence dir: benchmarks/detect/evidence/target-optimization-discrimination-20260714-13e6fff/.
- Impact: DECODE_FRAME_PIPELINE remains the accepted primary bottleneck, but no targeted optimization can be safely selected for Step 18.5. Step 18.4 remains in_progress; Step 18.5 remains planned/blocked.
- Resolution/Status: OPEN. The evidence gap is that the production HW decode path (pyav_iter_frames with stream.codec_context) produces non-reproducible pixel values across alternative decode paths, blocking exact-parity optimization candidates that require seeking or alternative container opens. Additionally, no bounded memory-retention model or PyAV configuration variant addresses the bottleneck. A future resolution might require: (1) understanding why stream.codec_context access changes HW decode output, (2) making the HW decode path reproducible across container opens, or (3) finding a completely different optimization class.
