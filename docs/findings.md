...
- Resolution/Status: PARTIALLY_RESOLVED. Step 18.4A probe (2026-07-13) confirms: (1) production pyav_iter_frames requests CUDA HWAccel via _pick_hw_device() → _create_hwaccel() → av.open(hwaccel=hwaccel), (2) stream.codec_context.is_hwaccel reports True, (3) hw_config_device_type=null and hw_config_format=null in the observed MPEG-4 test fixture, indicating no actual hardware decoder configuration. Per the direct installed-PyAV signal (is_hwaccel), runtime_hwaccel_active=True. However, the null config suggests the HWAccel was configured at the container level but no HW decoder surfaces were allocated for this codec (MPEG-4 has no CUDA decoder). The canonical H.264 Hermes clip was not available for testing — H.264 with CUDA may produce non-null hw_config. The probe mechanism works and captures complete structured evidence from the production path. F-0098 is considered PARTIALLY_RESOLVED: the observability gap is closed, but the discrimination between "HWAccel configured" and "HW decode active" for the actual H.264 Hermes clip requires the canonical clip.
- LINKS: M-DETECT-PERF-DECISION, V-PERF-DETECT-BOTTLENECK, Phase-18/Step-18.4, pyav_backend.py, benchmarks/detect/evidence/hwaccel-runtime-evidence/runtime_evidence.json

### F-0099 — Remaining optimization-discrimination gap after HWAccel runtime evidence
- Date: 2026-07-13
- Area: detection
- Finding: Step 18.4A confirms production path requests CUDA HWAccel and is_hwaccel=True. The primary bottleneck class DECODE_FRAME_PIPELINE (172.46s, 63.27%) remains accepted with HIGH confidence. However, no single targeted optimization can yet be selected. The rejected PYAV_HWACCEL_CUDA is confirmed unnecessary (source already requests HWAccel). Available candidates (SECOND_PASS_DECODE_ELIMINATION_OR_REUSE, PYAV_DECODE_CONFIGURATION_TUNING) lack sufficient design evidence or bounded analysis. The remaining discrimination gap is: which bounded decoder-pipeline optimization can be safely selected without causing quality regression or unsupported architectural change.
- Symptom/Reproduction: Step 18.4 candidate_evaluation lists all candidates with supporting/counter evidence. Bottleneck_decision.json selected_optimization=NONE. Step 18.5 remains planned/blocked.
- Impact: Step 18.4 remains in_progress. Step 18.5 cannot begin. V-PERF-DETECT-BOTTLENECK contract not yet satisfied.
- Resolution/Status: OPEN. Planned next evidence step: design analysis of SECOND_PASS_DECODE_ELIMINATION_OR_REUSE (memory vs time tradeoff, representative timestamp semantics, signature preservation proof) as the most promising candidate.
- LINKS: M-DETECT-PERF-DECISION, V-PERF-DETECT-BOTTLENECK, F-0098, benchmarks/detect/evidence/bottleneck_decision.json
