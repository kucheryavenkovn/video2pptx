...
- Impact: All corrected candidate discriminators have valid terminal measurements. C2 taxonomy corrected from resource model failure to performance failure. No targeted optimization satisfies exact semantics plus the stable >=15% performance gate. The canonical signature provenance gap (8cc06c6a... vs 5a7c4538...) blocks the exact-semantics gate until resolved. Outcome PENDING (terminal_outcome=null); terminal T3 is NOT accepted while the gate is open. selected_optimization=NONE; Step 18.4C=blocked_on_canonical_signature_provenance; Step 18.4=in_progress; Step 18.5=planned/blocked and not started.
- Resolution/Status: OPEN. Two paths to unblock: (1) run a controlled causal A/B probe that varies ONLY stream.codec_context access in pyav_iter_frames (removing it is NOT sufficient by itself, because its causality is not yet proven), or (2) make an architectural decision on the production decode path and then explicitly, consciously re-baseline the immutable signature under the current code. The evidence_observer added to slide_detector.py receives mutable cropped ndarray and must be documented as a trusted intrusive diagnostic hook.
