...
- Date: 2026-07-14
- Area: detection, tooling
- Finding: Corrected Step 18.4C-r2 established REFERENCE_EXACTLY_REPEATABLE across three fresh complete 1,201-frame production-equivalent opens, so exact cross-open byte discrimination is valid. C1 is NOT_VIABLE_EXACT_PARITY_FAIL — root cause ISOLATED: stream.codec_context access in pyav_iter_frames (added between original benchmark HEAD acb424f and current code) changes NVDEC HW decode pixel output, making production decode path non-reproducible by alternative decode paths. C2 is NOT_VIABLE_PERFORMANCE_FAIL (corrected from NOT_VIABLE_RESOURCE_MODEL) — rolling-window exact model IS proven (peak 15 frames / 93,312,000 bytes), but candidate is slower (reference median 290.526s, candidate median 330.722s; median of paired differences -46.820s / -16.115%; difference of medians -40.196s / -13.835%; not directionally faster). C3 is NOT_VIABLE_PERFORMANCE_FAIL — thread_count_1/4/8 selected and pass exact sequence parity but fail performance (median paired reductions -2.029%, -4.571%, +0.944%; none directionally faster or >= 15%). Canonical signature provenance: fresh runs produce 5a7c4538... while accepted immutable is 8cc06c6a...; root cause is the same stream.codec_context access change, NOT an environment anomaly.
...
- Impact: All corrected candidate discriminators have valid terminal measurements. C2 taxonomy corrected from resource model failure to performance failure. No targeted optimization satisfies exact semantics plus the stable >=15% performance gate. The canonical signature provenance gap (8cc06c6a... vs 5a7c4538...) blocks the exact-semantics gate until resolved. Outcome T3_blocked. selected_optimization=NONE; Step 18.4C=blocked_on_canonical_signature_provenance; Step 18.4=in_progress; Step 18.5=planned/blocked and not started.
- Resolution/Status: OPEN. Two paths to unblock: (1) remove stream.codec_context access from the hot decode path in pyav_iter_frames and re-baseline, or (2) accept the 5a7c4538... signature as the new immutable baseline under the current code. The evidence_observer added to slide_detector.py receives mutable cropped ndarray and must be documented as a trusted intrusive diagnostic hook.
