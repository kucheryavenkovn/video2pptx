...
- Resolution/Status: PARTIALLY_RESOLVED (2026-07-14 canonical Hermes H.264 evidence run, outcome OUTCOME_D). Evidence mechanism implemented and corrected (ST-18.4A, 2026-07-13): (1) private evidence observer separating raw codec_context_is_hwaccel from actual_hardware_decode_active, (2) _create_hwaccel_with_evidence capturing creation errors, (3) deterministic generator cleanup, (4) provenance-validated probe (--accepted-base, --canonical-mode). Tests: 17/17 pass. Canonical Hermes H.264 clip (SHA256 dd9da344...) was located and the probe executed (see F-0100 RESOLVED). Canonical observations: requested_hw_device=cuda, hwaccel_object_created=true (no creation error), container_opened_with_hwaccel=true, raw codec_context_is_hwaccel=true, allow_software_fallback=true, codec_name=h264 (1920x1080, 60fps), first_frame_shape=[1080,1920,3], observation_consistency=true, status OBSERVATION_COMPLETE. However actual_hardware_decode_active=UNKNOWN_NOT_PROVEN by design (installed API does not expose actual HW decode vs configured HWAccel) and the strict fallback control (allow_software_fallback=false) was inconclusive (STRICT_PROBE_NO_FRAME, no error — see F-0101). Canonical evidence commit: 9b4cc54949537b2a36e70a0f98f1cb732d6688b2; evidence dir benchmarks/detect/evidence/hwaccel-runtime-hermes-h264-20260714. Gap narrowed (runtime evidence object now exists and is complete) but the activation-determination question remains open. selected_optimization=NONE; Step 18.4A=done; Step 18.4=in_progress; Step 18.5=planned/blocked.
- LINKS: M-DETECT-PERF-DECISION, V-PERF-DETECT-BOTTLENECK, F-0100, pyav_backend.py
...
- LINKS: M-DETECT-PERF-DECISION, V-PERF-DETECT-BOTTLENECK, F-0098, F-0101, tools/probe_pyav_hwaccel.py

### F-0101 — Strict fallback control inconclusive; direct HW-decode activation not directly proven
- Date: 2026-07-14
- Area: detection
- Finding: The canonical Hermes H.264 HWAccel runtime evidence (2026-07-14) confirmed HWAccel was requested (cuda), created without error, the container was opened with HWAccel, and the raw codec_context.is_hwaccel=true. However actual hardware decode activation could not be directly proven because (1) the production observer deliberately reports actual_hardware_decode_active=UNKNOWN_NOT_PROVEN (installed PyAV 18.0.0 API does not expose actual HW decode vs configured HWAccel; hw_config_present=false), and (2) the strict fallback control experiment (HWAccel("cuda",0) with allow_software_fallback=false) returned result=STRICT_PROBE_NO_FRAME with error_type=null and no exception on the canonical clip. The strict control decodes only the first demuxed packet before breaking, so for this H.264 stream the first packet yields no frame — making the discriminator inconclusive. It neither proves HW decode active (no frame produced under the strict-no-fallback regime) nor proves software fallback (no error).
- Symptom/Reproduction: tools/probe_pyav_hwaccel.py --canonical-mode on examples\hermes-0000-1000.mp4 (SHA dd9da344...) → strict_fallback_control.result = STRICT_PROBE_NO_FRAME, error_type=null. production-path observations still yield frames (allow_software_fallback=true).
- Impact: F-0098 can only be PARTIALLY_RESOLVED. No activation defect is directly proven, so PYAV_HARDWARE_DECODE_ACTIVATION_OR_FALLBACK_FIX is not viable. selected_optimization remains NONE; Step 18.4 remains in_progress; Step 18.5 remains planned/blocked. Genuinely distinct from F-0098 (which is the general runtime-evidence gap, now narrowed).
- Resolution/Status: OPEN. A future direct-activation experiment would need to decode multiple packets/frames under allow_software_fallback=false (or use an FFmpeg-level hwaccel pipeline probe) to produce a decisive frame-or-error result. Out of scope for Step 18.4A (canonical observation collection), which is complete.
- LINKS: M-DETECT-PERF-DECISION, V-PERF-DETECT-BOTTLENECK, F-0098, F-0100, tools/probe_pyav_hwaccel.py, pyav_backend.py
